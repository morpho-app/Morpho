...
public const val validUrl = "(" +
        ...
        ")" + ")"
public val urlRegex = validUrl.toRegex()
...
public val mentionRegex = validMention.toRegex()
public val whitespaceRegex = """/ +(?=\n)""".toRegex()
public val whitespaceEofRegex = """/\s+${'$'}| +(?=\n)""".toRegex()
public const val validMarkdownLink = """\[([^]]+)\]\((${validUrl})\)"""
public val markdownLinkRegex = validMarkdownLink.toRegex()
public val validDid = "(did:[a-z]+:[a-zA-Z0-9._:%-]*[a-zA-Z0-9._-])"
public val didRegex = validDid.toRegex()
