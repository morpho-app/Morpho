...
package com.morpho.butterfly.cbor
...
internal open class CborReader(
  ...
  override fun decodeElementIndex(descriptor: SerialDescriptor): Int {
    while (true) {
      ...
      val currentIndex = descriptor.getElementIndex(elemName)
      if (currentIndex == CompositeDecoder.UNKNOWN_NAME) {
        decoder.skipElement()
      } else {
        decodeByteArrayAsByteString = descriptor.isByteString(currentIndex)
        return currentIndex
      }
    }
  }
